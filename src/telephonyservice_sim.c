...
bool _service_pin2_status_query_cb(LSHandle *handle, LSMessage *message, void *user_data)
{
	...
	if (!service->driver || !service->driver->pin2_status_query) {
...
bool _service_pin1_change_cb(LSHandle *handle, LSMessage *message, void *user_data)
{
	...
	jvalue_ref pin_obj = NULL;
	const char *payload;
	raw_buffer oldpin_buf = { 0 };
	raw_buffer newpin_buf = { 0 };
...
cleanup:
	if (oldpin_buf.m_str)
		jstring_free_buffer(oldpin_buf);
	if (newpin_buf.m_str)
		jstring_free_buffer(newpin_buf);
	if (!jis_null(parsed_obj))
		j_release(&parsed_obj);
...
bool _service_fdn_status_query_cb(LSHandle *handle, LSMessage *message, void *user_data)
{
	...
	if (!service->driver || !service->driver->fdn_status_query) {
